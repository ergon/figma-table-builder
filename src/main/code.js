    ...
    if (msg.type === 'build-table') {
        ...
        if (textNode === undefined || textNode == null) {
            ...
            Promise.all([
                ...
                var headerCells = createTable(figma.currentPage.selection[0], msg.wrap_in_autolayout, msg.right_align_numbers, msg.data);
...
function createTable(original, wrapInAutoLayout, right_align_numbers, data) {
    var columnAutoLayouts;
    columnAutoLayouts = [];
    var headerCells = [];
    if (wrapInAutoLayout) {
...
function getFirstChildOfTypeText(sceneNode) {
    if ("children" in sceneNode) {
        return sceneNode.findOne(node => node.type === "TEXT");
    }
    else {
        return undefined;
    }
